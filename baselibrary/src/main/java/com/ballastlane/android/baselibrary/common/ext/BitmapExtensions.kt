...
fun Bitmap.decodeSampledBitmapFromFile(filePath: String, requiredWidth: Int, requiredHeight: Int): Bitmap {
    ...
    if (imageHeight > requiredHeight || imageWidth > requiredWidth) {
        ...
        while (halfHeight / sampleSize > requiredHeight * imageMaxFactor ||
                halfWidth / sampleSize > requiredWidth * imageMaxFactor) {
            sampleSize *= 2
        }
    }
    ...
    Timber.d(TAG, "Decoding bitmap on path: " + filePath +
            " with sample size of: " + sampleSize +
